...
def show_start_window():
    ...
    label = tk.Label(root, text="Sistem Deteksi Pose Tari",
                     font=big_font, fg='white', bg='black')
    label.pack(pady=100)
...
def detectPose(image, pose, display=True):
    ...
    if results.pose_landmarks:
        ...
        style_titik = mp_drawing.DrawingSpec(
            color=(0, 0, 255), thickness=2, circle_radius=2)

        # 2. Warna GARIS (Hubungan antar titik) -> Misal: Putih (255, 255, 255)
        style_garis = mp_drawing.DrawingSpec(
...
def show_start_window():
    ...
    title_font = font.Font(family='Helvetica', size=36 if len(
        judul_program) < 60 else 28, weight='bold')
