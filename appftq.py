...
def classify_pose(angles):
    ...
    # ============================================
    #  CONTOH RULE: MENEPuk PAHA (FORMAT SESUAI)
    # ============================================
...
def video_loop(label_var, canvas):
    global running, cap
...
def start_camera(label_var, canvas):
    global running, cap
    ...
    threading.Thread(target=video_loop, args=(label_var, canvas), daemon=True).start()
...
def show_start_window():
    ...
    label_var.set("Belum Terdeteksi")

    lbl = tk.Label(root, textvariable=label_var, fg="white", bg="black", font=big_font)
    lbl.pack(pady=10)

    canvas = tk.Canvas(root, width=960, height=540, bg="gray")
    canvas.pack()

    start_btn = tk.Button(root, text="Start", command=lambda: start_camera(label_var, canvas))
    start_btn.pack(side=tk.LEFT, padx=20)

    stop_btn = tk.Button(root, text="Stop", command=stop_camera)
